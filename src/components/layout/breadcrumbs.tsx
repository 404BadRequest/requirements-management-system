...
/** Segmentos de ruta sin página propia; se muestran pero no enlazan (evita 404). */
const NON_NAVIGABLE_SEGMENTS = new Set(["id"]);
      ...
      {parts.length === 0 ? (
        ...
        <>
          ...
          {parts.map((segment, i) => {
            ...
            const isNavigable = !isLast && !NON_NAVIGABLE_SEGMENTS.has(segment);
            return (
              <span key={`${segment}-${i}`} className="flex items-center gap-1">
                ...
                {isLast ? (
                  ...
                  <span className="rounded-[2px] px-1.5 py-0.5 text-muted-foreground">{segmentLabel(segment)}</span>
                )}
              </span>
            );
          })}
